...
$url .= "&LM_MST_subdivY1VZ=" . str_replace(" ", "+", $subdivision);
...
while ($start > 0){
	...
	$html .= "mobileBaths='". getMobileBaths(substr($xml, $start, $end)) . "' ";
...
function getMobileBaths($xml){
	...
	$te = strpos($xml, "<", $ts2);
	$a1 = substr($xml, $ts2, $te-$ts2);
	$a1 = str_replace(" full, ", ".", $a1);
	$key = "<span class=hbaths>";
	$ts2 = strpos($xml, $key, $ts1) + strlen($key);
	$te = strpos($xml, "<", $ts2);
	$a2 = substr($xml, $ts2, $te-$ts2);
	$a2 = str_replace(" half", "", $a2);
	if ($a2 == "0 half")
		return trim(str_replace(",", "", $a1));
	else
		return trim($a1 . $a2);
